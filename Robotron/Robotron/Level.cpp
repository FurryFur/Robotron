	...
	for (size_t i = 0; i < m_scene.getEntityCount(); ++i)
	{
		...
		if (m_scene.getEntity(i).hasComponents(COMPONENT_SCOREPICKUP))
		{
			...
			if (m_scene.getEntity(i).aiVariables.followEntity != NULL)
			{
				...
				{
					m_scene.getEntity(i).aiVariables.followEntity->playerStats.playerInfo.addScore(m_scene.getEntity(i).aiVariables.score);
					m_scene.getEntity(i).aiVariables.followEntity->playerStats.extraLifeTrack += m_scene.getEntity(i).aiVariables.score;
				}
				else
					m_scene.getEntity(i).aiVariables.followEntity->playerStats.playerInfo.addLives(1);
			}
		...
		if (m_scene.getEntity(i).hasComponents(COMPONENT_PLAYER))
		{
			...
			if (m_scene.getEntity(i).hasComponents(COMPONENT_PLAYER) && m_scene.getEntity(i).playerStats.playerInfo.getLives() == 0)
				m_scene.getEntity(i).playerStats.playerInfo.addLives(1);
...
void Level::respawnDeadPlayers(Clock& clock)
{
	...
	for (unsigned int i = 0; i < m_scene.getEntityCount(); ++i)
	{
		if (m_scene.getEntity(i).hasComponents(COMPONENT_PLAYER)
		    ...
		    && m_scene.getEntity(i).playerStats.playerInfo.getLives() > 0
