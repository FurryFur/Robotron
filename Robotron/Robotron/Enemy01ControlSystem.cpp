...
void Enemy01ControlSystem::update(Entity& entity, float deltaTick)
{
	...
	{
		if ((m_scene.m_entities.at(i)->componentMask & COMPONENT_PLAYER_CONTROL) == COMPONENT_PLAYER_CONTROL						          //its a player object
			&& glm::length(m_scene.m_entities.at(i)->transform[3] - entity.transform[3]) < glm::length(targetPosition - currentPosition)     //it is the closet player to the target
			&& glm::length(m_scene.m_entities.at(i)->transform[3] - entity.transform[3]) < 30)								                  //the player is within the enemys aggro range
		{
			targetPosition = { m_scene.m_entities.at(i)->transform[3].x, m_scene.m_entities.at(i)->transform[3].y, m_scene.m_entities.at(i)->transform[3].z};
			targetVelocity = m_scene.m_entities.at(i)->physics.velocity;
			targetMoveSpeed = m_scene.m_entities.at(i)->controlVars.moveSpeed;
			targetFound = true;
		}
	}
	...
	if (targetFound)
	{
		...
		for (unsigned int i = 0; i < m_scene.m_entities.size(); ++i)
		{
			if (((m_scene.m_entities.at(i)->componentMask & COMPONENT_ENEMY01) == COMPONENT_ENEMY01) &&
				(glm::length(glm::vec2(m_scene.m_entities.at(i)->transform[3].x - entity.transform[3].x, m_scene.m_entities.at(i)->transform[3].z - entity.transform[3].z))) <= 2.0f)
			{
				nearbyNeighbours.push_back(m_scene.m_entities.at(i).get());
			}
		}
